...
cardano-cli transaction build-raw \
    ${tx_in} \
    --tx-out $(cat ${KEYS}/payment.addr)+${txOut} \
    --invalid-hereafter $(( ${currentSlot} + 10000)) \
    --fee ${fee} \
...
cardano-cli node key-gen \
    ...
    --operational-certificate-issue-counter ${COLD}/node.counter
...
cp ${COLD}/node.cert ${KEYS}/node.cert
...
cardano-cli transaction build-raw \
    ${tx_in} \
    --tx-out $(cat ${KEYS}/payment.addr)+$(( ${total_balance} - ${stakePoolDeposit}))  \
    --invalid-hereafter $(( ${currentSlot} + 10000)) \
    --fee 0 \
